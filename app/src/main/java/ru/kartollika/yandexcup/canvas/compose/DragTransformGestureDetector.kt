...
suspend fun PointerInputScope.detectDragOrTransformGestures2(
    panZoomLock: Boolean = false,
    onDragStart: (Offset) -> Unit = {},
    onDrag: (Offset) -> Unit = {},
    onDragRotate: (Pair<Float, Float>) -> Unit,
    ...
    awaitEachGesture {
        ...
        do {
            ...
            if (!canceled && gesturePointersRequirementMet && dragAmountThreshold > dragAmount) {
                ...
                val centroid = event.calculateCentroid(useCurrent = false)
                if (pos.x < (centroid.x + 100f) && pos.y < (centroid.y + 100f)) {

                    val prev = change.previousPosition
                    val dx = pos.x - prev.x
                    val dy = pos.y - prev.y
                    onDragRotate(Pair(dx, dy))
